...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function showNoResults() {
        ...
        if (window.innerWidth <= 1280) {
            ...
            mobile.querySelector(
                ".result2"
            ).innerHTML = `<b>No results found</b>`;
    ...
    searchBar.addEventListener("input", function () {
        ...
        debounceTimer = setTimeout(() => {
            ...
            if (query.length < 3) {
                suggestions.innerHTML =
                    "<li style='color: #aaa; padding: 0.75rem;'>Please enter at least 3 characters</li>";
    ...
    function updateSearchResult(office) {
        ...
        const infoHTML = `
            <div><span class="f1">Office</span><strong>${
                office.Name
            }</strong></div>
            <div><span class="f2">State</span><strong>${
                office.State
            }</strong></div>
            <div><span class="f1">Pincode</span><strong>${
                office.Pincode
            }</strong></div>
            <div><span class="f2">Country</span><strong>${
                office.Country
            }</strong></div>
            <div><span class="f1">Taluk</span><strong>${
                office.Taluk || "NA"
            }</strong></div>
            <div><span class="f2">Telephone No.</span><strong>${
                office.Telephone || "NA"
            }</strong></div>
            <div><span class="f1">Divison</span><strong>${
                office.Division
            }</strong></div>
            <div><span class="f2">Office Type</span><strong>${
                office.BranchType
            }</strong></div>
            <div><span class="f1">District</span><strong>${
                office.District
            }</strong></div>
            <div><span class="f2">Delivery Status</span><strong>${
                office.DeliveryStatus
            }</strong></div>
            <div><span class="f1">Region</span><strong>${
                office.Region
            }</strong></div>
            <div><span class="f2">Related sub office</span><strong>${
                office.SubOffice || "NA"
            }</strong></div>
            <div><span class="f1">Circle</span><strong>${
                office.Circle
            }</strong></div>
            <div><span class="f2">Related Head Office</span><strong>${
                office.HeadOffice || "NA"
            }</strong></div>
        `;
        ...
        if (isMobileScreen()) {
            ...
            mobile.querySelector(
                ".result2"
            ).innerHTML = `<b>${office.Name}</b> Pin Code is <b>${office.Pincode}</b>. ${office.Name} comes under <b>${office.District}</b> district.`;
        ...
        if (!canonicalLink) {
            const name = office.Name.split(" ")[0];
            canonicalLink = document.createElement("link");
            canonicalLink.setAttribute("rel", "canonical");
            canonicalLink.setAttribute(
                "href",
                `https://labh.io/pincode/?search=${name}&pin=${office.Pincode}`
            );
        ...
        setOrUpdateMeta(
            "og:title",
            `${office.Name} Pin Code | ${office.District}, ${office.State}`
        );
        setOrUpdateMeta(
            "og:description",
            `${office.Name} has a pin code of ${office.Pincode}. Located in ${office.District}, ${office.State}.`
        );
        ...
        setOrUpdateMeta(
            "twitter:image",
            "https://labh.io/assets/logo_final.png",
            false
        );
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const targetURL =
        "https://play.google.com/store/apps/details?id=com.labh.io&pcampaignid=web_share";
