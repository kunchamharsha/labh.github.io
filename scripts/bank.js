...
$(document).ready(function () {
  ...
  $(document).on("change", ".bank-switch", function () {
    const $el = $(this);
    const bankId = $el.data("bank-id");
    const isChecked = $el.is(":checked");

     updateSwitchImage($el, isChecked);
    ...
    $(".bank-switch")
      ...
      .each(function () {
        ...
        $(this)
          .closest(".img-switch")
          .find(".knob-image")
          .attr("src", `${ASSETS_URL}/assets/mobile-webview/not-verified.svg`);
      });

    setDefaultBank(bankId, isChecked, this);
  });
});
...
function updateSwitchImage($el, isVerified) {
  const $img = $el.closest(".img-switch").find(".knob-image");

  $img.attr(
    "src",
    isVerified
      ? `${ASSETS_URL}/assets/mobile-webview/verified.svg`
      : `${ASSETS_URL}/assets/mobile-webview/not-verified.svg`
  );
}
function setDefaultBank(bankId, isChecked, inputEl) {
  $.ajax({
    ...
    error: function (xhr, status, error) {
      ...
      inputEl.checked = !isChecked;
       updateSwitchImage($(inputEl), !isChecked);
    },
  });
}
        ...
        const bankCard = `
                    <div class="list">
                        <div class="l-header d-flex justify-content-between">
                            ...
                            <label class="img-switch">
                                <input 
                                    type="checkbox"
                                    class="bank-switch"
                                    data-bank-id="${account.id}"
                                    ${account.is_default === "Y" ? "checked" : ""}
                                >

                                <span class="slider">
                                    <img class="knob-image" src="${switchImage}">
                                </span>
                            </label>
                        </div>
                        <hr>
  ...
  $.ajax({
    ...
    error: function (error) {
      ...
      if (response.account_number) {
        ...
        if (
          response.non_field_errors == "You cannot add more than 5 accounts"
        ) {
          showErrors(
...
$(".modal-container").on("click", function (e) {
  if (!$(".c-modal").is(e.target) && $(".c-modal").has(e.target).length === 0) {
    hideModals();
  }
});
